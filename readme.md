...
이 문서는 이전 프로젝트에서 사용한 Gradle 멀티모듈 구조를 예시와 함께 설명합니다. 해당 프로젝트는 게시글과 사용자에 대한 생성, 수정, 조회 기능 예시를 포함하고 있습니다.

멀티 모듈 환경에서 작업하면서 든 애로사항은 서로 남이 만든 모듈의 내용이나 사용법을 파악하기 쉽지 않다는 거였어요. 
이를 해결하기 위해서, 모듈에서 노출할 api 를 인터페이스로 관리하고 그것을 기반으로 개발과 테스트하는 과정을 정리했습니다. 
...
- [추천하는 컨벤션](#추천하는-컨벤션)
  ...
  - [추천 컨벤션 2: testFixture 사용과 API 인터페이스의 Fake 객체로 테스트](#추천-컨벤션-2-testfixture-사용과-api-인터페이스의-fake-객체로-테스트)
    ...
    - [순수 자바로 테스트를 해야 하는 이유](#순수-자바로-테스트-하는-이유) 
      - [주입 시점에 익명클래스 or 람다를 넣어서 테스트하기 예시](#순수-자바로-테스트-하는-이유)
      - [순수 자바 테스트와 jpa 의존성을 slicing 테스트한 것의 속도차이 비교.](#순수-자바로-테스트-하는-이유)
  - [추천 컨벤션 3: testFixture의 관리와 전달](#추천-컨벤션-3-testfixture의-관리와-전달)
...
###  추천 컨벤션 1: 모듈에서 공개할 기능은 api/ 하위 경로의 인터페이스로 관리하기
...
개인적으로, 가능하면 하는 것이 좋다고 추천드리고 싶습니다.

이유는 여러가지가 있겠지만 가장 와닿는 이유는 협업하는 환경 & 모듈로 작업하는 환경에서는 **테스트가 없으면 리뷰어가 괴로워져요.**

타인이 만든 코드를 읽었을때, **구현부의 코드만 보고서는 이게 프로덕션에 반영될 만큼의 코드인지를 판단하기 힘듭니다.**

만약 테스트를 작성하는게 가성비가 안좋은것 같다라는 판단이 드신다면, 위의 구조로 개발을 할때 모듈 외부로 노출할 api 에 대해서만이라도 테스트를 작성을 해주시는 것을 추천드립니다.

사실 외부 노출할 api 에 대한 성공, 실패, 경계값을 테스트한다면 그 다음엔 코드 퀄리티정도만 확인을 하면 되서 팀 단위로 소모되는 에너지를 좀 아낄 수 있을 것이라 생각해요 .


사실 테스트를 해야 하는가? 얼마나 어떻게 해야 하는가? 는 팀마다 다르고 사람마다 생각이 다를 수 있는거라 여기에 대해서는 그냥 개인적인 생각이고 피드백, 의견 공유 환영합니다 : ) 